...
│                                                       │
│  ┌───────────────────────────────────────────────┐   │
│  │ OSRM サイドカー (osrm-backend:5000)            │   │
│  │ /table → N×N距離・所要時間行列                  │   │
│  │ /route → 実道路ルート形状 (GeoJSON)             │   │
│  │ exclude=motorway (高速道路回避)                  │   │
│  └───────────────────────────────────────────────┘   │
│                                                       │
...
| `AVG_SPEED_KMH` | 15 km/h | 市街地平均走行速度 (OSRM不可時のフォールバック) |
| `DEPOT_TURNAROUND_MINUTES` | 20分 | 便間のデポ帰還→再出発の所要時間 |
| `MAX_PACKAGES_PER_TRIP` | 50個 | 1便あたりの最大荷物数 |
| `OSRM_BATCH_SIZE` | 100 | OSRM `/table` APIの1バッチあたり最大ポイント数 |
| `ROUTE_SEGMENT_SIZE` | 25 | OSRM `/route` APIの1セグメントあたり最大ウェイポイント数 |
| `OSRM_URL` | `http://localhost:5000` (env) | OSRMサイドカーのURL (SPCS: localhost:5000, ローカル: :5001) |
...
5. **OSRM距離行列取得**: 全ポイント (デポ+荷物+ドライバーデポ) の座標をOSRM `/table` APIに投入し、N×N の実道路距離・所要時間行列を取得。500+ポイントの場合は `OSRM_BATCH_SIZE=100` で分割取得 (`sources`/`destinations` パラメータ使用)。全APIに `exclude=motorway` を付与し高速道路を回避。

#### コスト関数 (travelCost) — 3-tier フォールバック

```
travelCost(from, to):
  1. OSRM距離行列にエントリがある場合 → OSRM実道路距離 (km)
  2. H3R10セルが異なり、H3_COST_MATRIX にエントリがある場合 → TOTAL_COST
  3. 上記いずれもない場合 → Haversine距離 (直線距離)
```

#### 所要時間関数 (travelMinutes) — 2-tier フォールバック

```
travelMinutes(distKm, osrmMin):
  1. OSRM所要時間が利用可能 → そのまま使用 (分)
  2. OSRM不可 → distKm / AVG_SPEED_KMH × 60
```

OSRM有効時は実道路ネットワーク上の距離・所要時間を使用するため、運河・高速道路・一方通行を考慮した精度の高いルーティングが可能。
...
for each remaining package:
    dist = travelCost(current → package)          // OSRM → H3コスト行列 → Haversine
    travel = travelMinutes(dist, osrmDuration)     // OSRM所要時間 or dist/速度
...
最終配達先 → デポまでの距離 (OSRM優先) から帰着時刻を算出:
```
returnMinutes = lastStop.eta + DWELL_MINUTES + travelMinutes(travelCost(lastStop → depot), osrmDuration)
```

#### ルート形状取得 (Phase 2)

OSRM有効時、各便のルート (デポ→配達先群→デポ) をOSRM `/route` APIで実道路形状 (GeoJSON) として取得。
- `ROUTE_SEGMENT_SIZE=25` でセグメント分割
- 部分フォールバック: 失敗セグメントは直線座標で補完、成功セグメントは道路形状を維持
- `exclude=motorway` により高速道路を回避した一般道ルートを生成
- OSRM無効時はウェイポイント直線座標をそのまま使用
...
OSRMおよびSnowflakeの各データソースは独立した try-catch で保護されており、障害時も機能が段階的に縮退する:

| データソース | 利用不可時の動作 |
|---------------|----------------|
| `OSRM` | 距離: H3コスト行列→Haversineにフォールバック。所要時間: dist/AVG_SPEED_KMH。ルート形状: 直線座標 |
| `RISK_SCORES` | リスクスコアなし (null) で割当。ルート順序のリスク考慮が無効化 |
| `ABSENCE_PATTERNS` | 不在率なし (null) で割当。不在回避ペナルティが無効化 |
| `H3_COST_MATRIX` | OSRM有効時は影響なし。OSRM+コスト行列両方不可 → Haversineにフォールバック |

**全Snowflake障害時**: OSRM + Postgresデータで容量制約+エリア分散+実道路距離ベースのルート生成が可能。
**OSRM + 全Snowflake障害時**: Postgresデータのみで、Haversine距離ベースのルート生成が可能 (精度は低下)。
...
|------|-------------|---------------------------|
| 移動コスト | OSRM実道路距離 + H3_COST_MATRIX (3-tierフォールバック) | Haversine直線距離 (精度低) |
...
1. ~~**最近傍法の限界**~~: 2-opt 実装済み (A-1)。さらに Or-tools / CVRP ソルバーで大域的最適化が可能
2. **固定パラメータ**: DWELL_MINUTES が定数。recipient_type 別の滞在時間差分を反映可能。AVG_SPEED_KMH はOSRM有効時は未使用
...
7. **OSRM car.luaプロファイル**: `exclude=motorway` で高速道路回避済み。カスタムLuaプロファイルで配送トラック向けの重み付け (住宅街の狭路回避等) が可能
