...
Snowflake RISK_SCORES + ABSENCE_PATTERNS + H3_COST_MATRIX を参照し最適ルートを計算。
...
**データソース:** pgQuery (packages, drivers, construction) + sfQuery (RISK_SCORES, ABSENCE_PATTERNS, H3_COST_MATRIX)
...
```json
{
  ...
  "optimization_summary": { "cost_matrix_pairs": 1200, "risk_applied_count": 450 }
}
```
